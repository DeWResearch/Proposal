import { DaoMetadata, DaoUpdateDecodedParams, GrantPermissionDecodedParams, GrantPermissionWithConditionParams, InitializeFromParams, RegisterStandardCallbackParams, RevokePermissionDecodedParams, UpgradeToAndCallParams, WithdrawParams } from "../../types";
import { IClientDecoding } from "../interfaces";
import { ClientCore, DecodedApplyInstallationParams, DecodedApplyUninstallationParams, DecodedApplyUpdateParams, InterfaceParams } from "@aragon/sdk-client-common";
/**
 * Decoding module the SDK Generic Client
 */
export declare class ClientDecoding extends ClientCore implements IClientDecoding {
    /**
     * @param {data} Uint8Array
     * @return {DecodedApplyInstallationParams}
     * @memberof ClientDecoding
     */
    applyInstallationAction(data: Uint8Array): DecodedApplyInstallationParams;
    /**
     * @param {data} Uint8Array
     * @return {DecodedApplyInstallationParams}
     * @memberof ClientDecoding
     */
    applyUninstallationAction(data: Uint8Array): DecodedApplyUninstallationParams;
    /**
     * Decodes the apply update parameters from an encoded apply update action
     *
     * @param {Uint8Array} data
     * @return {DecodedApplyUpdateParams}
     * @memberof ClientDecoding
     */
    applyUpdateAction(data: Uint8Array): DecodedApplyUpdateParams;
    /**
     * Decodes the permission parameters from an encoded grant action
     *
     * @param {Uint8Array} data
     * @return {GrantPermissionDecodedParams}
     * @memberof ClientDecoding
     */
    grantAction(data: Uint8Array): GrantPermissionDecodedParams;
    /**
     * Decodes the grant permission with condition parameters from an encoded grant with condition action
     *
     * @param {Uint8Array} data
     * @return {GrantPermissionWithConditionParams}
     * @memberof ClientDecoding
     */
    grantWithConditionAction(data: Uint8Array): GrantPermissionWithConditionParams;
    /**
     * Decodes the permission parameters from an encoded revoke action
     *
     * @param {Uint8Array} data
     * @return {RevokePermissionDecodedParams}
     * @memberof ClientDecoding
     */
    revokeAction(data: Uint8Array): RevokePermissionDecodedParams;
    /**
     * Decodes the withdraw parameters from an encoded withdraw action
     *
     * @param {Uint8Array} data
     * @return {WithdrawParams}
     * @memberof ClientDecoding
     */
    withdrawAction(to: string, value: bigint, data: Uint8Array): WithdrawParams;
    /**
     * Decodes a dao metadata ipfs uri from an encoded update metadata action
     *
     * @param {Uint8Array} data
     * @return {string}
     * @memberof ClientDecoding
     */
    updateDaoMetadataRawAction(data: Uint8Array): string;
    /**
     * Decodes a dao metadata from an encoded update metadata raw action
     *
     * @param {Uint8Array} data
     * @return {Promise<DaoMetadata>}
     * @memberof ClientDecoding
     */
    updateDaoMetadataAction(data: Uint8Array): Promise<DaoMetadata>;
    /**
     * Decodes the daoUri from a setDaoUriAction
     *
     * @param {Uint8Array} data
     * @return {string}
     * @memberof ClientDecoding
     */
    setDaoUriAction(data: Uint8Array): string;
    /**
     * Decodes the RegisterStandardCallbackParams from a registerStandardCallbackAction
     *
     * @param {Uint8Array} data
     * @return {RegisterStandardCallbackParams}
     * @memberof ClientDecoding
     */
    registerStandardCallbackAction(data: Uint8Array): RegisterStandardCallbackParams;
    /**
     * Decodes the implementation address from an encoded upgradeToAction
     *
     * @param {Uint8Array} data
     * @return {string}
     * @memberof ClientDecoding
     */
    setSignatureValidatorAction(data: Uint8Array): string;
    upgradeToAction(data: Uint8Array): string;
    /**
     * Decodes upgradeToAndCallback params from an upgradeToAndCallAction
     *
     * @param {Uint8Array} data
     * @return {UpgradeToAndCallParams}
     * @memberof ClientDecoding
     */
    upgradeToAndCallAction(data: Uint8Array): UpgradeToAndCallParams;
    /**
     * Decodes the initializeFrom params from an initializeFromAction
     *
     * @param {Uint8Array} data
     * @return {InitializeFromParams}
     * @memberof ClientDecoding
     */
    initializeFromAction(data: Uint8Array): InitializeFromParams;
    /**
     * Returns the decoded function info given the encoded data of an action
     *
     * @param {Uint8Array} data
     * @return {(InterfaceParams | null)}
     * @memberof ClientDecoding
     */
    findInterface(data: Uint8Array): InterfaceParams | null;
    /**
     * Decodes the dao update params from a daoUpdateAction
     *
     * @param {Uint8Array} data
     * @return {DaoUpdateDecodedParams}
     * @memberof ClientDecoding
     */
    daoUpdateAction(data: Uint8Array): DaoUpdateDecodedParams;
}
